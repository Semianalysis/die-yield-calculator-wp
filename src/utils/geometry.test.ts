...
describe("geometry utils", () => {
	...
	describe("rectanglesInRectangle", () => {
		it("calculates the correct number of possible rectangles in another rectangle when starting from the top-left", () => {
			...
			expect(
				rectanglesInRectangle(60, 60, 20, 20, 0, 0, 0, 0, false, false)
					.positions.length,
			).toBe(9);
		});
		...
		it("calculates the correct number of rectangles when starting from the center", () => {
			expect(
				rectanglesInRectangle(60, 60, 20, 20, 0, 0, 0, 0, true, false).positions
					.length,
			).toBe(9);
			expect(
				rectanglesInRectangle(60, 60, 30, 30, 0, 0, 0, 0, true, false).positions
					.length,
			).toBe(4);
			// When the inner square is more than 1/2 the width or height of the outer square, it centers it correctly
			expect(
				rectanglesInRectangle(60, 60, 40, 40, 0, 0, 0, 0, true, false).positions
					.length,
			).toBe(1);
		});
		...
		it("calculates partial overlaps when starting from the top left", () => {
			...
			expect(
				rectanglesInRectangle(100, 100, 40, 40, 0, 0, 0, 0, false, true)
					.positions.length,
			).toBe(9);
			// Once the inner square areas divide evenly into the outer square area, the
			// partial overlaps are edged out
			expect(
				rectanglesInRectangle(100, 100, 50, 50, 0, 0, 0, 0, false, true)
					.positions.length,
			).toBe(4);
		});
		...
		it("calculates partial overlaps when starting from the center", () => {
			...
			expect(
				rectanglesInRectangle(100, 100, 90, 90, 0, 0, 0, 0, true, true)
					.positions.length,
			).toBe(4);
			...
			expect(
				rectanglesInRectangle(70, 70, 20, 20, 0, 0, 0, 0, true, true).positions
					.length,
			).toBe(16);
		});

		it("calculates the correct number of rows and columns", () => {
			const result = rectanglesInRectangle(
				90,
				100,
				10,
				10,
